...
def loadFeaturesForBook(filename, smartStopWords, pronSet, conjSet):
    ...
    wordsDist = MyFreqDist(FreqDist(allWords))
    ...
    wordLengthDist = map(lambda x: wordLengthDistribution.freq(x), range(4,18))
...
def withCrossFoldValidation(x, y, scoring):
    ...
    cval = StratifiedShuffleSplit(y, n_iter=NUMFOLDS, test_size=.30)
